...
using ProgressFunc = std::function<int(
    const TParameters &, const TSolutions &)>; // template aliasing.

template <class TParameters, class TSolutions>
using PlotFunc = std::function<int(const TParameters &, const TSolutions &,
...
public:
   ...
   */
  int optimize(const EvalFunc &evalf, const AskFunc &askf,
   ...
   */
  void set_plot_func(const PlotFunc<TParameters, TSolutions> &pffunc) {
  ...
  /**
   * \brief returns the numerical gradient of the objective function in
   * phenotype space
  ...
  /**
   * \brief uncertainty handling scheme that perform completely the reevaluation
   * of solutions.
   */
  void perform_uh(const dMat &candidates, const dMat &phenocandidates,
                  int &nfcalls);

  /**
   * \brief part of the ucertainty handling scheme that select which candidates
   * should be reevaluated.
   */
  void select_candidates_uh(const dMat &candidates, const dMat &phenocandidates,
                            dMat &candidates_uh);

  /**
   * \brief part of the ucertainty handling scheme that evaluate the candidates
   * to be reevaluated.
   */
  void eval_candidates_uh(const dMat &candidates, const dMat &candidates_uh,
                          std::vector<RankedCandidate> &nvcandidates,
                          int &nfcalls);

  /**
   * \brief part of the ucertainty handling scheme that set the results of
   * evaluation to the solutions.
   */
...
protected:
  ...
  TSolutions _solutions;   /**< holder of the current set of solutions and the
                              dynamic elemenst of the search state in general. */
  TParameters _parameters; /**< the optimizer's set of static parameters, from
                              inputs or internal. */
  ProgressFunc<TParameters, TSolutions>
      _pfunc;                /**< possibly custom progress function. */
  GradFunc _gfunc = nullptr; /**< gradient function, when available. */
  PlotFunc<TParameters, TSolutions>
      _pffunc; /**< possibly custom stream data to file function. */
  FitFunc _funcaux;
  bool _initial_elitist = false; /**< restarts from and re-injects best seen
